...
class YfinanceRepository:
    ...
    def get_price(self, tickers:list[str], date:date)-> list[Price]:
        ...
        for t in data.tickers.values():
            ...
            if df.empty:
                logging.error(f"No price data found for ticker {t.ticker} on date {date}")
                continue
            price = Price(
    ...
    #@st.cache_data(ttl=3600)
    def get_currency_conversion(self, from_currency:str, to_currency:str, date:date) -> float:
